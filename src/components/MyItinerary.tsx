...
export const MyItinerary: React.FC = () => {
  ...
  const availablePool: PoolItem[] = [
    // 1. 6 Days Main Itinerary
    ...MAIN_ITINERARY.map((d) => ({
      sourceKey: `main-day-${d.dayNumber}`,
      title: d.title,
      subtitle: d.subtitle,
      sourceType: 'main_day' as const,
      categoryLabel: `主行程 Day ${d.dayNumber}`,
      summary: d.theme,
      location: d.highlights.slice(0, 3).join(' ➔ '),
      timeRange: '全日精華',
      tips: d.weatherAdvice,
      rawDayData: d
    })),
    // 2. Day 5 Plan B
    {
    ...
    // 3. Alternative Styles Day Modules
...
// Helper to check where an item sourceKey is placed and its completion status
  // Supports canonical mapping (e.g. 'main-day-1' matches 'main-day-1', 'fixed-d1-flight', etc.)
  const getItemPlacementInfo = (sourceKey: string, currentTargetDay: number | null) => {
    // 1. Determine the canonical source identifiers to check against
    const keysToCheck = [sourceKey];
    if (sourceKey === 'main-day-1') {
      keysToCheck.push('fixed-d1-flight', 'main-day-1-explore');
    } else if (sourceKey === 'main-day-4') {
      keysToCheck.push('main-day-4-daytime', 'fixed-d4-dragon-bridge');
    } else if (sourceKey === 'main-day-6') {
      keysToCheck.push('main-day-6-museum', 'fixed-d6-flight');
    }

    for (const day of containers) {
      // Find any item in the day that matches either exact sourceKey, parent sourceKey, or sourceDay
      ...
      if (match) {
        // If there are multiple items for this source in the day, check if any is completed
  ...
  const getRawDetailsForDetailItem = () => {
    ...
    if (item.sourceKey === 'fixed-d1-flight' || item.sourceKey === 'main-day-1') {
      return { type: 'day' as const, data: MAIN_ITINERARY[0] };
    }
    if (item.sourceKey === 'main-day-2') {
      return { type: 'day' as const, data: MAIN_ITINERARY[1] };
    }
    if (item.sourceKey === 'main-day-3') {
      return { type: 'day' as const, data: MAIN_ITINERARY[2] };
    }
    if (item.sourceKey === 'fixed-d4-dragon-bridge' || item.sourceKey === 'main-day-4-daytime' || item.sourceKey === 'main-day-4') {
      return { type: 'day' as const, data: MAIN_ITINERARY[3] };
    }
    if (item.sourceKey === 'main-day-5') {
      return { type: 'day' as const, data: MAIN_ITINERARY[4] };
    }
    if (item.sourceKey === 'fixed-d6-flight' || item.sourceKey === 'main-day-6-museum' || item.sourceKey === 'main-day-6') {
      return { type: 'day' as const, data: MAIN_ITINERARY[5] };
    }
    ...
    if (item.sourceKey.startsWith('main-day-')) {
      ...
      if (rawDay) return { type: 'day' as const, data: rawDay };
    }
